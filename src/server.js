...
function leadPrompt(service, lang, heaterType) {
  const title = lang === 'en' ? `✅ Selected: ${serviceName(service, lang)}` : `✅ Servicio: ${serviceName(service, lang)}`;
  const typeLine = service === 'calentador' && heaterType
    ? (lang === 'en' ? `✅ Heater type: ${heaterType}` : `✅ Tipo: ${heaterType}`)
    : '';
...
function parseLeadMessage(text) {
  ...
  for (const l of lines) {
    ...
    if (m3) {
      ...
      if (phone && !phone.startsWith('+')) phone = '+1' + phone.replace(/\D/g, '');
      out.phone = phone;
      continue;
    }
  ...
  if (out.details.length > 700) out.details = out.details.slice(0, 700);
...
const handler = async (req, res) => {
  try {
    ...
    if (session.step === 'menu') {
      ...
      session.heater_type = 'N/A';
      await saveSession(key, session);
      return res.status(200).type('text/xml').send(twiml(leadPrompt(service, session.lang, '')));
    }

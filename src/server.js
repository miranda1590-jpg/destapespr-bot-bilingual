...
function detectLanguage(text, currentLang) {
  ...
  if (['hello', 'hi', 'hey', 'english', 'emergency', 'emergencies', 'urgent', 'asap', 'price', 'cost', 'estimate', 'clog', 'leak', 'heater', 'camera', 'appointment'].some(w => lower.includes(w))) return 'en';
...
function detectIntent(text) {
  const lower = norm(text);
  if (['emergencia', 'urgencia', 'urgente', 'emergency', 'emergencies', 'urgent', 'asap'].some(w => lower.includes(w))) return 'emergencia';
...
async function getEmergencySlots(forTomorrow = false) {
  ...
  try {
    ...
    slots = slots.filter(s => !bookedSet.has(s.start_iso));
...
function residentialMenuText(lang) {
  if (lang === 'en') return `🏠 Is it a House or an Apartment/Condo?\n\n1️⃣ House\n2️⃣ Apartment / Condo\n\nReply with 1 or 2.`;
  return `🏠 ¿Es una Casa o un Apartamento/Condominio?\n\n1️⃣ Casa\n2️⃣ Apartamento / Condominio\n\nResponde con 1 o 2.`;
}
...
const handler = async (req, res) => {
  ...
  if (session.step === 'menu') {
    ...
    if (selectedService) {
      ...
      // LA MAGIA: En lugar de ir directo al Lead, pregunta si es Residencial o Comercial
    ...
    if (isComercial) {
        session.property_type = session.lang === 'en' ? 'Commercial' : 'Comercial';
        session.service_label += ` (${session.property_type})`;
        ...
        session.step = 'residential_type';
        responseMsg = residentialMenuText(session.lang);
    } else {
        responseMsg = propertyMenuText(session.lang);
    }
  }
  else if (session.step === 'residential_type') {
    const isCasa = ['1', 'casa', 'house'].some(w => lower.includes(w));
    const isApto = ['2', 'apartamento', 'condominio', 'apartment', 'condo'].some(w => lower.includes(w));
    
    if (isCasa || isApto) {
        session.residential_type = isCasa ? (session.lang === 'en' ? 'House' : 'Casa') : (session.lang === 'en' ? 'Apt/Condo' : 'Apto/Condominio');
        session.service_label += ` (${session.property_type} - ${session.residential_type})`;

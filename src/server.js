...
const TAG = process.env.TAG || "DestapesPR Unified Bot 🇵🇷 [[FORCE-20260211-080251-DEPLOY]]";
...
async function initDB() {
  ...
  if (!has('awaiting_heater_type')) await db.exec(`ALTER TABLE sessions ADD COLUMN awaiting_heater_type INTEGER DEFAULT 0;`);
...
async function saveSession(from, patch = {}) {
  ...
  await db.run(
    `
    INSERT INTO sessions (from_number, lang, case_id, last_choice, awaiting_heater_type, awaiting_details, awaiting_schedule, awaiting_slot, heater_type, details, slots_json, last_active)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
function matchService(bodyRaw) {
  ...
  for (const key of SERVICE_KEYS) {
    if (SERVICE_KEYWORDS[key].some(w => txt.includes(w))) return key;
  }
  return null;
}
...
function heaterTypePrompt(lang) {
  if (lang === 'en') {
    return `✅ Service: Water heater (gas/electric/solar)\n\nBefore details, choose type:\n1️⃣ Solar\n2️⃣ Conventional (gas/electric)\n\nReply 1 or 2.`;
  }
  return `✅ Servicio: Calentador (gas/eléctrico/solar)\n\nAntes de los detalles, elige tipo:\n1️⃣ Solar\n2️⃣ Convencional (gas/eléctrico)\n\nResponde con 1 o 2.`;
}

function detailsPrompt(service, lang, heaterType) {
  const baseEN =
  ...
  const examplesEN = {
    ...
    calentador: `"I'm Ana Rivera, 939-555-9999, Caguas, ${heaterType === 'SOLAR' ? 'solar water heater' : 'water heater'} not heating"`,
    cita: `"I'm Ana Rivera, 939-555-9999, Caguas, prefer Tue 2–4pm, kitchen sink clogged"`,
  ...
  const examplesES = {
    ...
    calentador: `"Me llamo Ana Rivera, 939-555-9999, Caguas, ${heaterType === 'SOLAR' ? 'calentador solar' : 'calentador'} no calienta"`,
    cita: `"Me llamo Ana Rivera, 939-555-9999, Caguas, prefiero martes 2–4pm, fregadero tapado"`,
...
function askSchedule(lang) {
  if (lang === 'en') return `📅 Would you like to schedule an appointment now?\n\nReply:\n✅ YES\n❌ NO`;
  return `📅 ¿Quieres agendar una cita ahora?\n\nResponde:\n✅ SI\n❌ NO`;
}
...
async function scriptPost(payload) {
  ...
  try { return JSON.parse(txt); } catch { return { ok: false, error: 'non_json', raw: txt }; }
}
...
function extractName(text) {
  ...
  const first = parts[0] || '';
  return first.replace(/^"|"$/g, '').replace(/^me llamo\s+/i, '').replace(/^i'?m\s+/i, '').trim();
}
...
app.post('/webhook/whatsapp', async (req, res) => {
  ...
  if (isFirstTime || isInactive || !bodyNorm || isMenuCommand || isHello) {
  ...
  if (session.awaiting_heater_type && session.last_choice === 'calentador') {
    if (bodyNorm === '1' || bodyNorm.includes('solar')) {
      ...
      return sendTwilioXML(res, `✅ Tipo: SOLAR\n\n` + detailsPrompt('calentador', lang, 'SOLAR'));
    }
    if (bodyNorm === '2' || bodyNorm.includes('convencional') || bodyNorm.includes('gas') || bodyNorm.includes('electrico') || bodyNorm.includes('eléctrico')) {
      session = await saveSession(from, {
        heater_type: 'Convencional',
        awaiting_heater_type: 0,
        awaiting_details: 1
      });
      return sendTwilioXML(res, `✅ Tipo: Convencional\n\n` + detailsPrompt('calentador', lang, 'Convencional'));
    }
    return sendTwilioXML(res, heaterTypePrompt(lang));
  }
  ...
  if (session.awaiting_slot && session.slots_json) {
    ...
    if (!pick || pick < 1 || pick > slots.length) return sendTwilioXML(res, formatSlots(slots, lang));
    ...
    const bookPayload = await scriptPost({
      ...
      heater_type: session.last_choice === 'calentador' ? (session.heater_type || 'N/A') : 'N/A',
    ...
    const slotLabel = (lang === 'en') ? chosen.slot_en : chosen.slot_es;

    const msg = bookedOk
      ? ((lang === 'en')
          ? `✅ Appointment booked.\nCase: ${session.case_id}\nService: ${serviceName(session.last_choice, lang)}\nSlot: ${chosen.ymd} — ${slotLabel}\n\n📞 ${PHONE}\n📘 ${FB_LINK}`
          : `✅ Cita agendada.\nCaso: ${session.case_id}\nServicio: ${serviceName(session.last_choice, lang)}\nCita: ${chosen.ymd} — ${slotLabel}\n\n📞 ${PHONE}\n📘 ${FB_LINK}`)
      : ((lang === 'en')
          ? `✅ Saved, but booking failed.\nCase: ${session.case_id}\nType "menu".`
          : `✅ Guardado, pero falló la reserva.\nCaso: ${session.case_id}\nEscribe "menu".`);
  ...
  if (session.awaiting_schedule && session.case_id && session.last_choice && session.details) {
    ...
    if (no) {
      ...
      const msg = (lang === 'en')
        ? `✅ Saved. Case: ${session.case_id}\nService: ${serviceName(session.last_choice, lang)}\n\n📞 ${PHONE}\n📘 ${FB_LINK}`
        : `✅ Guardado. Caso: ${session.case_id}\nServicio: ${serviceName(session.last_choice, lang)}\n\n📞 ${PHONE}\n📘 ${FB_LINK}`;
    ...
    if (!avail?.ok || !Array.isArray(avail.slots) || avail.slots.length === 0) {
      ...
      const msg = (lang === 'en')
        ? `⚠️ No slots available now. Saved.\nCase: ${session.case_id}\nType "menu".`
        : `⚠️ No hay horarios ahora. Guardado.\nCaso: ${session.case_id}\nEscribe "menu".`;
  ...
  if (svc) {
    ...
    return sendTwilioXML(res, detailsPrompt(svc, lang, null));
  }
